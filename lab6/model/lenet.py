...
class LeNetNumpy(object):
    def __init__(self, matmul: str):
        ...
        self.conv1 = Conv2D(self.w1, self.b1, self.conv1_quantization_params, pad="SAME", matmul=self.matmul)
        self.conv2 = Conv2D(self.w2, self.b2, self.conv2_quantization_params, pad="VALID", matmul=self.matmul)
        self.dense3 = Dense(self.w3, self.b3, self.dense3_quantization_params, matmul=self.matmul)
        self.dense4 = Dense(self.w4, self.b4, self.dense4_quantization_params, matmul=self.matmul)
        self.dense5 = Dense(self.w5, self.b5, self.dense5_quantization_params, matmul=self.matmul)

    def forward(self, x):
        # [TODO] 使用定义后的网络层算子实现网络前向传播
        # 例：
        conv1_out = self.conv1(x)
        pool1_out = self.maxpooling(conv1_out)
        conv2_out = self.conv2(pool1_out)
        pool2_out = self.maxpooling(conv2_out)
        flattenned = self.flatten(pool2_out)
        dense3_out = self.dense3(flattenned)
        dense4_out = self.dense4(dense3_out)
        dense5_out = self.dense5(dense4_out)
        output = dense5_out
        return output
